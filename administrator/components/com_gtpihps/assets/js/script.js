...
function submitbuttonDelete(task, form) {
	if (confirm(Joomla.JText._('COM_GTPIHPS_CONFIRM_DELETE'))) {
		submitform(task, form);
	}
	return false;
}
...
var loadOptions = function(target, task, data, first_row) {
	var first_row = typeof first_row !== 'undefined' ? first_row : false;
	var first_opt = jQuery('option:first-child', target).clone();
	var parameters = {"task": task};
	jQuery.extend(parameters, data);

	target.attr('disabled', true);
	jQuery.ajax({url: component_url, data: parameters, success: function(result){
		target.removeAttr('disabled');
		target.html(result).trigger("liszt:updated");

		if(first_row) {
			target.prepend(first_opt);
			target.val(first_opt.val());
		}
		target.change();
	}});
}
