...
class GTHelperFlot {
	...
	static function loadChart($id, $data, $labels, $options){
		self::loadLibrary();
		...
		$chart_height	.= 'px';
		$data_array		= array_slice($data, 0, 1);
		$dates			= $data ? array_keys(array_shift($data_array)): array();
		...
		$js_chart_download_button = $download_button ? "
			$('#chart-download-$id').click(function(){
				var obj = $('#$id');
				var modal = $('#chart-modal-$id');

				var flotText = $('div.flot-text', obj);
				flotText[0].style.setProperty('color', '#000', 'important');

				html2canvas(obj, {
					onrendered: function(canvas){
						flotText[0].style.setProperty('color', '#000');

						var dt = canvas.toDataURL('image/png');
						$('.modal-body', modal).html('<img src=\"' + canvas.toDataURL('image/png') + '\">');
						$('.modal-dialog', modal).width(canvas.width + 20);
						modal.modal('show');
					}
				});
			});
		...
		$output .= $download_button ? sprintf('
			...
			<div class="modal fade" tabindex="-1" role="dialog" id="chart-modal-%s">
				<div class="modal-dialog">
					<div class="modal-content">
						<div class="modal-header text-left">%s</div>
						<div class="modal-body"></div>
						<div class="modal-footer">
							<button type="button" class="btn btn-danger" data-dismiss="modal">%s</button>
						</div>
					</div>
				</div>
			</div>
		', $id, $id, JText::_('COM_GTPIHPS_IMG_DOWNLOAD_INFO'), JText::_('COM_GTPIHPS_TOOLBAR_CLOSE'), JText::_('COM_GTPIHPS_TOOLBAR_CLOSE')) : NULL;
		$output .= '</div>';
	...
	static function loadLibrary(){
		$document = JFactory::getDocument();
		$component_assets_uri = GT_ADMIN_URI . '/assets';
	...
	static function setData($data, $labels, $options){
	...
	static function setOptions($options){
		...
		$opt['grid']['backgroundColor'] = 'rgba(44, 62, 80, 0.4)';
		$opt['grid']['borderColor'] = '#2c3e50';

		$opt['legend']['show'] = isset($options->show_legend) && in_array($options->show_legend, array(1,2));

		$opt['xaxis']['mode'] = 'time';
		$opt['xaxis']['color'] = '#2c3e50';
		$opt['xaxis']['axisLabelUseCanvas'] = FALSE;
		...
		$opt['yaxis']['color'] = '#2c3e50';
		$opt['yaxis']['axisLabelUseCanvas'] = FALSE;
		$not_money = isset($options->not_money) && $options->not_money;
		if(!$not_money) {
