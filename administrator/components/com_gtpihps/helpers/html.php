...
class GTHelperHTML
{
	...
	static function loadHeaders() {
		...
		// Add Styles
		$document->addStylesheet(GT_ADMIN_CSS . '/style.css');
		...
		JText::script('COM_GTPIHPS_CONFIRM_DELETE');
	...
	static function setCommodities($rows, $categories, $commodities, $catIds = array(), $level = 0) {
		$data = array();
		foreach($rows as $category_id => $category) {
			...
			if(count($child_categories)) {
				...
				if(!($countsub || count($child_commodities || in_array($category_id, $catIds)))) {
					continue;
				}
			} else {
				if(!(count($child_commodities) || in_array($category_id, $catIds))) {
					continue;
				}
			}
			
			$row				= new stdClass();
			$row->text			= str_repeat('&nbsp;', $level * 6) . $category->name;
			$row->value			= 'cat-'.$category_id;
			$row->id			= $category_id;
			$row->name			= $category->name;
			$row->denomination	= $category->denomination;
			$row->type			= 'category';
			$row->level			= $level;
			$data[]				= $row;
			if(isset($commodities[$category_id])) {
				foreach($commodities[$category_id] as $commodity_id => $commodity) {
					$multiplier			= $level + 1;
					$row				= new stdClass();
					$row->text			= str_repeat('&nbsp;', $multiplier * 6) . $commodity->name;
					$row->value			= $commodity_id;
					$row->id			= $commodity_id;
					$row->name			= $commodity->name;
					$row->denomination	= $commodity->denomination;
					$row->type			= 'commodity';
					$row->level			= $multiplier;
					$data[]				= $row;
				}
			}
			$row = new stdClass();
			if(isset($categories[$category_id])) {
				$data = array_merge($data, self::setCommodities($categories[$category_id], $categories, $commodities, $catIds, $level+1));
			}
		}
		return $data;
	}
