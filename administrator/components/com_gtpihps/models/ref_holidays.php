...
class GTPIHPSModelRef_Holidays extends GTModelList{
	...
	public function __construct($config = array()) {
		if (empty($config['filter_fields'])) {
			$config['filter_fields'] = array('a.name', 'a.start', 'a.end');
		}
	...
	protected function getListQuery() {
		...
		// Select data.
		$query->select($db->quoteName(array('a.id', 'a.name', 'a.start', 'a.end', 'a.published')));
		...
		if (!empty($search)) {
			...
			$search_query[] = $db->quoteName('a.start') . 'LIKE ' . $search;
			$search_query[] = $db->quoteName('a.end') . 'LIKE ' . $search;
