...
class GTPIHPSModelRef_National_Commodities extends GTModelList{
	...
	public function __construct($config = array()) {
		if (empty($config['filter_fields'])) {
			$config['filter_fields'] = array('a.name', 'b.name', 'a.denomination');
		}
	...
	protected function getListQuery() {
		...
		// Select data.
		$query->select($db->quoteName(array('a.id', 'a.name', 'a.denomination', 'a.published')));
		$query->select($db->quoteName('b.name', 'category'));
		$query->from($db->quoteName('#__gtpihps_national_commodities', 'a'));
		$query->join('LEFT', '#__gtpihps_categories AS b ON a.category_id = b.id');
