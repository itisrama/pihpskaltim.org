...
class GTHelperFieldset {
	...
	public static function tplEditField2($label, $input, $class = null) {
		$str = '
			<div class="form-group %class">
				%label
				%input
	...
	public static function tplViewTable($thead, $tbody, $condensed = false) {
	...
	public static function renderEdit($fieldset, $type = 2) {
		$editField = "";
		$editFieldset = "";
	...
	public static function renderSubView($field, $showNum = true, $condensed = false) {
		...
		if(!$data) {
			$html = '<p>'.JText::_('COM_GTRESTO_NO_DATA').'</p>';
			return $html;
		}
		...
		$thead = array();
		if($showNum) {
			$thead[] = '<th class="text-center" width="40px">'.JText::_('COM_GTRESTO_NUM').'</th>';
		}
		...
		if(is_object($form)) {
			foreach ($keys as $k => $key) {
				...
				if(is_object($field)) {
					...
					$subFields[$k]['label']		= JText::_('COM_GTRESTO_FIELD_'.strtoupper($key));
				}
			...
			foreach ($keys as $k => $fl) {
				...
				$subFields[$k]['label']		= $notranslate == 1 ? str_replace('_', ' ', $name) : JText::_('COM_GTRESTO_FIELD_'.strtoupper($name));
