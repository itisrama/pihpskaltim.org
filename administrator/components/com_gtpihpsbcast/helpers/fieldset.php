...
class GTHelperFieldset {
	...
	public function tplEditField($label, $input, $class = null) {
		$str = '
	...
	public function tplEditFieldset($fieldset) {
		$str = '
	...
	public function tplViewField($num, $label, $input, $showNum = true) {
	...
	public function tplViewFieldset($tbody, $class = '') {
		$str = '
	...
	public function renderEdit($fieldset) {
		$editField = "";
		$editFieldset = "";
	...
	public function renderSubView($field, $showNum = true, $condensed = false) {
		...
		if(!$data) {
			$html = '<p>'.JText::_('COM_GTPIHPSBCAST_NO_DATA').'</p>';
			return $html;
		}
		...
		$thead = array();
		if($showNum) {
			$thead[] = '<th class="text-center" width="40px">'.JText::_('COM_GTPIHPSBCAST_NUM').'</th>';
		}
		...
		if(is_object($form)) {
			foreach ($keys as $k => $key) {
				...
				if(is_object($field)) {
					...
					$subFields[$k]['label']		= JText::_('COM_GTPIHPSBCAST_FIELD_'.strtoupper($key));
				}
			...
			foreach ($keys as $k => $fl) {
				...
				$subFields[$k]['label']		= $notranslate == 1 ? str_replace('_', ' ', $name) : JText::_('COM_GTPIHPSBCAST_FIELD_'.strtoupper($name));
