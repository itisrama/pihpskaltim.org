...
class JFormFieldTableGridFo extends JFormField
{
	...
	protected function getInput() {
		...
		if(!$disableAdd) {
			$str[] = '<button id="' . $id . '_toggle" class="btn btn-primary" type="button">';
			$str[] = '<i class="fa fa-plus"></i> ' . str_replace('%s', $labeltext, JText::_('COM_JKCOMMODITY_MANAGER_NEW'));
			$str[] = '</button>';
		}
		...
		foreach ($fields as $field) {
			...
			$str[]	= JKHelperFieldset::tplEditField($label, $input, 'inputField'.$resetClass);
		}
		
		$str[] = '</div>';
		$str[] = '<div class="modal-footer">';
		$str[]	= '<button class="addItem btn btn-primary" type="button"><i class="fa fa-plus"></i> ' . JText::_('COM_GTJOBORDER_ADD_ITEM') . '</button>';
		$str[]	= '<button class="saveItem btn btn-primary" type="button" style="display:none"><i class="fa fa-save"></i> ' . JText::_('COM_GTJOBORDER_SAVE_ITEM') . '</button>';
		$str[]	= '<button class="cancelItem btn btn-warning" type="button"><i class="fa fa-times"></i> ' . JText::_('COM_GTJOBORDER_TOOLBAR_CLOSE') . '</button>';
