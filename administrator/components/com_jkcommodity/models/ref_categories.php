...
class JKCommodityModelRef_Categories extends JKModelList{
	...
	public function __construct($config = array()) {
		if (empty($config['filter_fields'])) {
			$config['filter_fields'] = array('a.name', 'b.name', 'a.type');
		}
	...
	protected function getListQuery() {
		...
		// Select data.
		$query->select($db->quoteName(array('a.id', 'a.name', 'a.type', 'a.published')));
		$query->select($db->quoteName('b.name', 'parent'));
		$query->from($db->quoteName('#__jkcommodity_category', 'a'));
		$query->join('LEFT', '#__jkcommodity_category AS b ON a.parent_id = b.id');
