...
class JKCommodityModelRef_City_Commodities extends JKModelList{
	...
	public function __construct($config = array()) {
		if (empty($config['filter_fields'])) {
			$config['filter_fields'] = array('a.name', 'a.type', 'a.key', 'a.published');
		}
	...
	protected function getListQuery() {
		...
		// Select data.
		$query->select($db->quoteName(array('a.id', 'a.name', 'a.type', 'a.key', 'a.published', 'a.created_by')));
		...
		if (!empty($search)) {
			...
			$search_query[] = $db->quoteName('a.type') . 'LIKE ' . $search;
			$search_query[] = $db->quoteName('a.key') . 'LIKE ' . $search;
