...
class JKCommodityModelRef_Commodities extends JKModelList{
	...
	public function __construct($config = array()) {
		if (empty($config['filter_fields'])) {
			$config['filter_fields'] = array('a.name', 'a.short_name', 'b.name', 'a.denomination');
		}
	...
	protected function getListQuery() {
		...
		// Select data.
		$query->select($db->quoteName(array('a.id', 'a.name', 'a.short_name', 'a.denomination', 'a.published')));
		$query->select($db->quoteName('b.name', 'category'));
		$query->from($db->quoteName('#__jkcommodity_commodity', 'a'));
		$query->join('LEFT', '#__jkcommodity_category AS b ON a.category_id = b.id');
		...
		if (!empty($search)) {
			...
			$search_query[] = $db->quoteName('a.denomination') . 'LIKE ' . $search;
			$search_query[] = $db->quoteName('a.original_name') . 'LIKE ' . $search;
