...
class GTPIHPSModelPrice extends GTModelAdmin{
	...
	public function getForm($data = array(), $loadData = true, $control = 'jform') {
		...
		$id = $data->id > 0 ? $data->id : $this->item->id;
		if($id > 0) {
			$form->setFieldAttribute('province_id', 'readonly', 'true');
			$form->setFieldAttribute('regency_id', 'readonly', 'true');
			$form->setFieldAttribute('market_id', 'readonly', 'true');
			$form->setFieldAttribute('price_type_id', 'readonly', 'true');
	...
	public function getItem($pk = null) {
		...
		$market		= $this->getItemExternal($data->market_id, 'market'); 
		$data->price_type_id = $market->price_type_id;
	...
	public function getMarketBySource($province_id, $market_id) {
		// Get a db connection.
		$db = $this->_db;
	...
	public function getMarkets($all = false) {
		$regency_id = $this->input->get('regency_id');
		$price_type_id = $this->input->get('price_type_id');
		...
		if(!$this->user->authorise('core.admin')){
			...
			if(!empty($province_list)){
				$query->where($db->quoteName('a.province_id').' IN ('.$province_list.')');
			}
			else{
				$query->where($db->quoteName('a.province_id').' = 0');
			}
		}
	...
	public function getMarketOptions() {
		return GTHelperArray::toOption($this->getMarkets());
	}
