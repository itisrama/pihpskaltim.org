...
class GTPIHPSModelPrices extends GTModelList{
	...
	public function getItems($table = false) {
		$items = parent::getItems($table);
		foreach ($items as &$item) {
			$item->regency = sprintf(JText::_('COM_GTPIHPS_'.strtoupper($item->regency_type)), trim($item->regency));
		}
		return $items;
	}
	...
	protected function getListQuery() {
		...
		$query->select($db->quoteName('b.type', 'regency_type'));
