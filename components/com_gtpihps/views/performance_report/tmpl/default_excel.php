...
$selectedProv = count($selectedProv) > 0 ? $selectedProv : array(JText::_('COM_GTPIHPS_ALL_PROVINCES'));
...
$objPHPExcel->getActiveSheet()->getStyle('A'.$rowNum.':'.'M'.$rowNum)->applyFromArray($style->bold);
...
$objPHPExcel->getActiveSheet()->getStyle('A'.($rowDataStart-1).':'.'M'.$rowNum)->getAlignment()->setWrapText(true);
...
$objPHPExcel->getActiveSheet()->getStyle('A'.($rowDataStart-1).':'.'M'.$rowNum)->applyFromArray($styleBorder);
...
$objPHPExcel->getActiveSheet()->getStyle('A'.($rowDataStart-1).':'.'M'.($rowDataStart-1))->applyFromArray(
