		...
		foreach ($refProvinces as $province_id => $province) {
			...
			foreach ($provRegencies as $regency_id => $regency) {
				...
				foreach ($regMarkets as $market_id => $market) {
					...
					$mItem->desc	= sprintf('%s%% (%s/%s)', round(($marketCount/$dayCount)*100), $marketCount, $dayCount);
				...
				$rItem->desc		= sprintf('%s%% (%s/%s)', round(($countDayRegency/$dayCount)*100), $countDayRegency, $dayCount);
			...
			$pItem->desc		= sprintf('%s%% (%s/%s)', round(($countDayProvince/$dayCount)*100), $countDayProvince, $dayCount);
