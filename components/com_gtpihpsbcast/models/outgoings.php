...
class GTPIHPSBCastModelOutgoings extends GTModelList
{
	...
	public function __construct($config = array()) {
		if (empty($config['filter_fields'])) {
			$config['filter_fields'] = array('a.id', 'a.transaction_id', 'a.name', 'a.phone', 'a.message', 'a.sent_time');
		}
	...
	protected function getListQuery() {
		...
		// Select project
		$query->select($db->quoteName(array('a.id', 'a.transaction_id', 'a.name', 'a.msisdn', 'a.message', 'a.sent_time')));
		$query->from($db->quoteName('#__gtpihpsbcast_outgoings', 'a'));
		...
		if (!empty($search)) {
			...
			$search_query[] = $db->quoteName('a.msisdn') . 'LIKE ' . $search;
