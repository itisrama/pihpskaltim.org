...
class GTPIHPSBCastModelSchedulers extends GTModelList
{
	...
	protected function getListQuery() {
		...
		// Select project
		$query->select($db->quoteName(array('a.id', 'a.schedules')));
		$query->from($db->quoteName('#__gtpihpsbcast_schedulers', 'a'));

		// Join group
		$query->select('GROUP_CONCAT('.$db->quoteName('b.name').') groups');
		$query->join('LEFT', $db->quoteName('#__gtpihpsbcast_groups', 'b') . ' ON ' .
			$db->quoteName('b.id') . 'IN (IF(' . $db->quoteName('a.group_ids') . ' = "", 0, '.$db->quoteName('a.group_ids').'))');
