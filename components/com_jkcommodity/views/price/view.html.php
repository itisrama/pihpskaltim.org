...
class JKCommodityViewPrice extends JKView {
	...
	public function display($tpl = null) {
		...
		$this->jinput->set('market_id', $this->item->market_id);
		$this->jinput->set('unit_id', $this->item->unit_id);
		$this->last_prices	= $this->get('LastPrices');
		...
		$this->document->addScript( JK_JS . '/script_price.js' );
	...
	protected function makeButtons() {
		...
		if ($isNew) {
			...
			if ($canCreate) {
				$buttons[] = JKHelper::makeButton('apply', 'price.apply');
				$buttons[] = JKHelper::makeButton('save', 'price.save');
				$buttons[] = JKHelper::makeButton('save2new', 'price.save2new');
			}
			$buttons[] = JKHelper::makeButton('cancel', 'price.cancel');
		} else {
			...
			/*
			if ($canCreate) {
				$buttons[] = JKHelper::makeButton('save2copy', 'price.save2copy');
			}*/
			$buttons[] = JKHelper::makeButton('cancel', 'price.cancel');
			if ($canEditState) {
				$buttons[] = JKHelper::makeButton('trash', 'prices.trash', true);
			}
