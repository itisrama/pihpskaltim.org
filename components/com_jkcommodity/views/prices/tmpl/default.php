...
<div id="com_jkcommodity" class="item-page<?php echo $this->params->get('pageclass_sfx'); ?>">
	...
	<form action="<?php echo JRoute::_('index.php?option=com_jkcommodity&view=prices'); ?>" method="post" name="adminForm" id="adminForm" class="form-vertical">
		<?php if($this->buttons):?>
			<div class="well">
		...
		<fieldset class="filter open">
			...
			<div class="container-fluid row-fluid">
				...
				<div class="span4">
					<div class="control-group">
						<label for="filter_market_id" class="control-label">
							<?php echo JText::_('COM_JKCOMMODITY_LABEL_MARKET')?>
						</label>
						<div class="controls">
							<?php echo JHtml::_('select.genericlist', $this->markets, 'filter_market_id[]', 'size="6" multiple="multiple"', 'value', 'text', $this->state->get('filter.market_id'));?>
						</div>
					</div>
				</div>
				<div class="span3">
		...
		<table class="adminlist table table-striped table-bordered table-hover">
			<thead>
				<tr>
					...
					<th width="17%" style="text-align:center">
						<?php echo JHtml::_('grid.sort', 'COM_JKCOMMODITY_FIELD_UNIT_LABEL', 'un.name', $listDirn, $listOrder); ?>
					</th>
					<th width="17%" style="text-align:center">
						<?php echo JHtml::_('grid.sort', 'COM_JKCOMMODITY_FIELD_CITY_LABEL', 'ct.name', $listDirn, $listOrder); ?>
					</th>
					<th width="17%" style="text-align:center">
						<?php echo JHtml::_('grid.sort', 'COM_JKCOMMODITY_FIELD_MARKET_LABEL', 'mk.name', $listDirn, $listOrder); ?>
					</th>
					<th width="17%" style="text-align:center">
			...
			<tbody>
				...
				<?php foreach ($this->items as $i => $item) : ?>
					<?php $editUrl = JRoute::_('index.php?option=com_jkcommodity&task=price.edit&id='.(int) $item->id); ?>
					<tr class="row<?php echo $i % 2; ?>">
						...
						<td>
							<a href="<?php echo $editUrl; ?>">
								<?php echo $this->escape($item->market); ?>
							</a>
						</td>
						<td style="text-align:center">
							<a href="<?php echo $editUrl; ?>">
